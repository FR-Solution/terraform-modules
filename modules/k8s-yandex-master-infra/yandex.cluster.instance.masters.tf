...
#     for_each = { for k, v in local.current_subnet  : k => v if v == try(var.master_group.resources_overwrite.group["${split("-", each.key)[0]}-${split("-", each.key)[2]}"].zone, var.master_group.default_zone)}
#     content {
#       subnet_id = yandex_vpc_subnet.master-subnets[network_interface.key].id
