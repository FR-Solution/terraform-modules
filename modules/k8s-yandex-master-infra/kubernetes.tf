...
resource "null_resource" "cluster" {
  ...
  provisioner "local-exec" {
    command = "kubectl config set-cluster  ${var.k8s_global_vars.cluster_name} --server=https://${tolist(tolist(yandex_lb_network_load_balancer.api-external.listener)[0].external_address_spec)[0].address} --insecure-skip-tls-verify && kubectl --request-timeout=5m cluster-info"
  }
}
